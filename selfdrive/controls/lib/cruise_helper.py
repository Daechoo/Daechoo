...
#   - 엑셀을 밟았다 떼면: zz
  ...
  def update_speed_nda(self, CS, controls):
    ...
    apply_limit_speed, road_limit_speed, left_dist, first_started, max_speed_log = \
      road_speed_limiter.get_max_speed(cluster_speed, True, self.autoNaviSpeedCtrlStart, self.autoNaviSpeedCtrlEnd)
  ...
  def check_gas_cruise_on(self, CS, v_cruise_kph):
    ...
    #### CruiseON상태이면
      ...
      if self.xState == XState.softHold:
        pass # longActiveUser = -2 (벌트는 롱컨 오토홀드시 인게이지유지가 요구됨)
      #  2. 신호감지감속중: cruiseOFF: 신호감지감속이 맘에 안드는 상태, 가속페달을 밟으면 해제
